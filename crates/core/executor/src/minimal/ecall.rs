...
pub(super) extern "C" fn sp1_ecall_handler(ctx: *mut sp1_jit::JitContext) -> u64 {
    ...
    let result = ecall_handler(ctx, code).expect("todo: handle interrupt");
...
pub fn ecall_handler(ctx: &mut impl SyscallContext, code: SyscallCode) -> Result<u64, Interrupt> {
    ...
    match code {
        ...
        SyscallCode::DELETE_PROFILER_SYMBOLS => Ok(delete_profile_symbols_syscall(ctx, arg1, arg2)),
        SyscallCode::VERIFY_SP1_PROOF
        | SyscallCode::COMMIT
        | SyscallCode::COMMIT_DEFERRED_PROOFS => Ok(None),
    }.map(|r| r.unwrap_or(code as u64))
}
