...
use crate::worker::{
    ...
    FinalVmStateLock, ProofData, SpawnProveOutput, TaskContext, TouchedAddresses, TouchedPages,
    ...
    W: WorkerClient,
{
    async fn call(&self, input: SplicingTask) -> Result<(), ExecutionError> {
        ...
        join_set.spawn_blocking(move || {
            ...
            loop {
                ...
                match vm.execute()? {
                    ...
                    CycleResult::Done(true) => {
                        ...
                        // this value.
                        let final_state = FinalVmState::from_splicing_vm_enum(&vm);
            ...
            // Append the touched pages from this chunk to the globally tracked touched pages.
            tracing::debug_span!("collecting touched pages and sending to global memory").in_scope(
                || {
                    all_touched_pages
                        .blocking_extend(touched_pages.is_set())
                        .map_err(|e| ExecutionError::Other(e.to_string()))
                },
            )?;
            Ok(())
