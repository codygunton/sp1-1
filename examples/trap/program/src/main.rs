...
pub fn main() {
    ...
    const PERMS: [u8; 4] = [PROT_NONE, PROT_READ, PROT_WRITE, PROT_READ | PROT_WRITE];
    ...
    // U256x2048 Mul
    let pages4 = [p1, p2, p3, p4];
    let data4: [&[u8]; 4] = [&DEFAULT_64, &DEFAULT_256, &DEFAULT_256, &DEFAULT_64];

    for &perms in &four_page_perms {
        test_four_pages(pages4, data4, perms, run_u256x2048_mul);
    }
